...
const Win = (props) => {
    ...
    const handleDownload = () => {
        html2canvas(document.getElementById('levelCompletionBox')).then((canvas) => {
         ...
         <div className='modal'>
            ...
            <Confetti/>
                <div className='win_con popup_con' id="levelCompletionBox">
                    ...
                    <button className='popup_button sb primary' onClick={()=>{navigate(props.src)}}>Play Next level</button>
                    </div>
                    <div className='home_div'>
                    <div className='go_to_home' onClick={()=>{navigate(-1)}}>Go to Home</div> 
                    <div className='share' onClick={toggleOptions}></div>
                    </div> 
                    {showOptions && (
                    <div className='share_con'>
                    <button className='sm download' onClick={handleDownload}></button>
                    <button className='sm facebook' onClick={handleShareFacebook}></button>
                    <button className='sm instagram' onClick={handleShareInstagram}></button>
                    <button className='sm whatsapp' onClick={handleShareWhatsApp}></button>
                    </div>
                    )}
                </div>
            </div>
